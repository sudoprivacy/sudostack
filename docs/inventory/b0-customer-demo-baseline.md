...
| Contract baseline | Immutable C metadata remains `staged`/`unfrozen` with empty support arrays; package-external A verifies admission only for the included Moss/`ZoneId` embedded boundary | Confirmed-Code; scope-bounded |
| F1 Contract artifact | `@sudo/contracts@0.2.1` content C `273fd409…`; `candidate_unpublished` | Confirmed-Code |
...
| Confirmed Contract boundary | Exact C consumed by Moss M and integrated as `18a0a06…`; `ZoneId` only at `NexusManager.start()` for embedded Nexus; immutable binding, one `--cluster-init` argument, rejection/no-side-effect tests, installed package proof, default CI and unchanged package bytes are verifier-bound | Confirmed-Code; package-external admission, not deployment evidence |
...
| Immutable successor content | private package `@sudo/contracts@0.2.1`; C `273fd4097cbc33c1c049c39bb1fb60cef2663e2b`, merged as `dc9cf01acf61dff586d292d15710332d35a5f811`; runtime/Schema/fixture/validator/owner-source bytes equal the immutable `0.2.0` baseline | Confirmed-Code; C package metadata remains `unfrozen`, empty support, `candidate_unpublished`, `not_deployed` | [content-stage record](../design/contracts-0.2.1-content-stage.md), [staged candidate manifest](../../manifests/releases/0.2.1-candidate.gen.json) |
| Included `0.2.1` consumer for `ZoneId` | feature `sudoprivacy/moss@e9660ed1483cf01f96fe06c45ba7e070e0223ec3`, merged to `dev` as `18a0a069b808c295287675afc6346f411be971a5` | Confirmed-Code; PR 278 exact-pins C; installed package, unchanged `NexusManager.start()` boundary, rejection/no-side-effect behavior, external exclusion and default CI verified; no release/deployment claim | [Moss PR 278](https://github.com/sudoprivacy/moss/pull/278), [package-external activation/support record](../../manifests/operations/0.2.1-activation-support.json) |

The G3 **included consumer target is Moss/`ZoneId` only**. Historical standalone verification still binds the `0.2.0` PR 276 evidence and C-04 resolution. For `0.2.1`, package-external A independently binds exact C, Moss M, integration parents and bytes, the installed package and lock, unchanged production source/caller tests, default runner/workflows, and exact hosted checks. Operator configuration remains the authoritative input writer; Moss validates and binds the value and produces the embedded launch argument; Nexus persisted topology remains the canonical runtime store. External topology is excluded. C's package remains byte-identical to what M consumed and continues to embed null/pending/empty staged metadata. C-03 is resolved only for the included Moss/`ZoneId` embedded boundary by A; this does not make C package-wide frozen, published, or deployed. `ResourceRef` has no production provider/consumer and remains excluded, as do runtime `ZonePath`, SudoWork, sudocode, and `sudowork-server`. No live B0, deployment, mixed-version, or rollback gate is satisfied by this code evidence.
...
| E-02 | SudoStack baseline `93e72878fc7b246d7f7f2418ff19ac41a02b5a66` preserves the exact activated/available `0.2.0` candidate and standalone historical support evidence. Immutable `0.2.1` content C is `273fd409…`, integrated as `dc9cf01…`; A adds only package-external evidence. | Confirmed-Code | [historical consumer-support operation](../../manifests/operations/consumer-support.json); [content-stage record](../design/contracts-0.2.1-content-stage.md); [A record](../../manifests/operations/0.2.1-activation-support.json) |
| E-03 | The historical `0.2.0` package and immutable `0.2.1` C package retain their original lifecycle/support bytes. C remains `unfrozen` with empty embedded support; A records separate included-scope admission without rewriting the package. | Confirmed-Code | [historical candidate manifest](../../manifests/releases/0.2.0-candidate.gen.json); [staged candidate manifest](../../manifests/releases/0.2.1-candidate.gen.json); [A record](../../manifests/operations/0.2.1-activation-support.json) |
...
| E-09 | SudoStack requires Node `>=22.21.0`; Moss M exact-pins C, verifies installed `0.2.1`, and passed lint, server-tests, and amd64 build at M; integration push server-tests/build also passed. The actual running runtime remains Unknown. | Confirmed-Code | [`package.json`](../../package.json); [Moss PR 278 checks](https://github.com/sudoprivacy/moss/pull/278/checks); [A record](../../manifests/operations/0.2.1-activation-support.json) |
...
| E-12 | SudoStack includes deterministic package pack/install/import/typecheck smoke. Moss M adds an installed-C identity/vector/package smoke at the default server-test runner while preserving the real caller and deployment-surface tests. These are executable code-level checks, not evidence that the selected customer environment passed. | Confirmed-Code | [package smoke](../../tools/contracts/package-smoke.mjs); [A evidence](../../manifests/operations/0.2.1-activation-support.json) |
...
| E-14 | Package-external A binds exact C `273fd409…`, Moss M `e9660ed…`, merge `18a0a06…`, all 43 C package paths and tarball identity, the exact three-file repin diff, preserved production boundary, default runner/workflows, and hosted checks. It changes no C package byte and requires no second repin. | Confirmed-Code | [A record](../../manifests/operations/0.2.1-activation-support.json); [A verifier](../../tools/contracts/activation-0.2.1.mjs); [Moss PR 278](https://github.com/sudoprivacy/moss/pull/278) |
...
| C-03 | Historical `0.2.0` support and immutable `0.2.1` C-stage metadata had empty embedded support, so the exact candidate-specific Moss pin and admission evidence were absent. | Resolved only for the included Moss/`ZoneId` embedded boundary: A verifies C `273fd409…`, M `e9660ed…`, integration `18a0a06…`, installed `0.2.1`, unchanged production semantics, default CI and all C package bytes. C remains staged/unfrozen internally; no package-wide freeze, release, deployment or assembly claim. | [A record](../../manifests/operations/0.2.1-activation-support.json); [Moss PR 278](https://github.com/sudoprivacy/moss/pull/278) |

C-04 remains resolved by its historical production-caller evidence. C-03 is resolved only for the included Moss/`ZoneId` embedded boundary by the verified package-external A record binding exact C and M; C's package bytes and embedded staging metadata remain unchanged. Assembly and G3 remain blocked by the 13 Unknown live inputs, built-artifact identity, mixed-version smoke, and exact rollback rehearsal. A does not change `candidate_unpublished`, `not_deployed`, `ResourceRef`/`ZonePath` deferral, or the zero `Confirmed-Deployment` count.
...
|---|---|---|
| SudoStack/F1 | Historical baseline `5a2a531…` preserves private `@sudo/contracts@0.2.0`; immutable C `273fd409…` supplies private `0.2.1`; package-external A verifies candidate-specific support without changing C. No executable selected-profile deployment or live artifact digest is present. | Candidate source identity and included-scope code admission are Confirmed-Code; package/image/binary deployment identity remains Unknown. |
| Moss | Base inspection anchor `be38bec235c4dea202e32791f4efd20529e18d20` declares package `2.1.88`, independent server/runtime release identities, PostgreSQL migrations `1`–`4`, and installer rollback of config/current symlink—not a database rollback. M `e9660ed…`, integrated as `18a0a06…`, is the included `0.2.1` consumer and binds exact C at the embedded production startup boundary. | Record the running Moss revision, installed Contract integrity/digest, server and separate session runtimes, image digest, applied migration state, config reference, actual boundary smoke, and DB restore evidence. [Package](https://github.com/sudoprivacy/moss/blob/18a0a069b808c295287675afc6346f411be971a5/package.json), [A evidence](../../manifests/operations/0.2.1-activation-support.json) |
...
9. the C-04 historical resolution operation and its exact Moss revisions, paths, hashes, `0.2.0` pin, boundary assertions, and hosted-check evidence still pass in the immutable historical checkout; package-external A continues to verify exact C+M, unchanged C package bytes, the `0.2.1` installed boundary and no second repin; this scoped C-03 resolution cannot substitute for any remaining live, assembly, mixed-version or rollback proof;
