...
| Target Contract consumer | Moss, `ZoneId` production argument-construction boundary; PR-head tests cover the helper, but the production startup call does not pass `clusterInit` | Conflicting; assembly-blocking |
...
The source changes referenced below are open pull requests. Code existence, tests, package installation, or a green pull request does not establish that a customer environment runs those bytes. Only evidence gathered from the selected live environment may use `Confirmed-Deployment`.
...
|---|---|---|---|
| E-01 | This inventory worktree is based on SudoStack `44b4e9de087e0b4908ab8f1af407737b435b4104`; that tree declares `@sudo/contracts` `0.1.0` and contains no checked-in deploy/profile/submodule spine. | Confirmed-Code | [`package.json` at the base revision](https://github.com/sudoprivacy/sudostack/blob/44b4e9de087e0b4908ab8f1af407737b435b4104/package.json); [base tree](https://github.com/sudoprivacy/sudostack/tree/44b4e9de087e0b4908ab8f1af407737b435b4104) |
| E-02 | PR 18 carries the proposed B0/B1/rollback clauses and the generated candidate/source-closure manifests. | Confirmed-Code | [ADR-005 at PR head](https://github.com/sudoprivacy/sudostack/blob/aec52e9dc5438946dd83d5c56c07149a4800120a/docs/adr/ADR-005-product-contract-versioning.md); [source closure](https://github.com/sudoprivacy/sudostack/blob/aec52e9dc5438946dd83d5c56c07149a4800120a/manifests/source-closure.gen.json) |
| E-03 | The candidate records exact owner revisions and reports `candidate_unpublished` plus `not_deployed`; actual producer/consumer declarations in the candidate remain empty. | Confirmed-Code | [candidate manifest](https://github.com/sudoprivacy/sudostack/blob/aec52e9dc5438946dd83d5c56c07149a4800120a/manifests/releases/0.2.0-candidate.gen.json) |
| E-04 | Nexus VFS supplies owner-local `ZoneId`/strict `ZonePath` sources and conformance artifacts at its PR head. | Confirmed-Code | [nexus-vfs PR-head tree](https://github.com/nexi-lab/nexus-vfs/tree/24f6730ec90fab8a035b2f5400ed313bda343fbe/contracts) |
| E-05 | Nexus supplies the draft-frozen `ResourceRef` owner source, but this initiative has no production `ResourceRef` caller. | Confirmed-Code | [Nexus owner manifest](https://github.com/nexi-lab/nexus/blob/759842551697efa2579a92f24b16f8ae43829e7b/src/nexus/contracts/schemas/common/v1/resource-ref.manifest.json); [Nexus PR 4801](https://github.com/nexi-lab/nexus/pull/4801) |
...
| E-07 | Cloud, Private, and Edge are expected Atlas-hosting profiles sharing the SudoStack spine; Local explicitly has no Atlas. | Expected | [`README.md` profile model](https://github.com/sudoprivacy/sudostack/blob/44b4e9de087e0b4908ab8f1af407737b435b4104/README.md#L137-L223) |
...
| E-09 | PR 18 requires Node `>=22.21.0` and records CI Node `22.21.0`; Moss PR 272 aligns supported server build/deploy paths to Node `22.22.1`, but the actual running runtime remains Unknown. | Confirmed-Code | [F1 package metadata](https://github.com/sudoprivacy/sudostack/blob/aec52e9dc5438946dd83d5c56c07149a4800120a/package.json#L1-L9); [Moss PR 272 files](https://github.com/sudoprivacy/moss/pull/272/files) |
| E-10 | Neither main nor PR 18 contains executable Cloud/Private/Edge/Local profile manifests. The older unmerged deploy branch contains only three scaffold files and leaves deploy, real session-pod health, fresh-host e2e, and rollback automation absent/TODO. | Legacy-Declared | [`feat/deploy-automation@662ecd49`](https://github.com/sudoprivacy/sudostack/tree/662ecd49de552e245a9efaccc73904c067d01d01/deploy); [TODO checks](https://github.com/sudoprivacy/sudostack/blob/662ecd49de552e245a9efaccc73904c067d01d01/deploy/README.md#L49-L61) |
...
| E-12 | PR 18 includes temporary-directory package pack/install/import/typecheck smoke, and Moss includes packaged-server and runtime-image smoke implementations. These are executable code-level checks, not evidence that the selected customer environment passed. | Confirmed-Code | [F1 package smoke](https://github.com/sudoprivacy/sudostack/blob/aec52e9dc5438946dd83d5c56c07149a4800120a/tools/contracts/package-smoke.mjs#L13-L151); [Moss release smoke wiring](https://github.com/sudoprivacy/moss/blob/be38bec235c4dea202e32791f4efd20529e18d20/.github/workflows/build-release.yml#L141-L226) |
| C-01 | At PR 18 head, ADR-005 still says a release manifest/full closure does not exist while generated candidate/closure manifests exist in the same revision. | Conflicting | [ADR-005](https://github.com/sudoprivacy/sudostack/blob/aec52e9dc5438946dd83d5c56c07149a4800120a/docs/adr/ADR-005-product-contract-versioning.md#L223-L225); [candidate manifest](https://github.com/sudoprivacy/sudostack/blob/aec52e9dc5438946dd83d5c56c07149a4800120a/manifests/releases/0.2.0-candidate.gen.json) |
| C-02 | At PR 18 head, the `ResourceRef` README reports remote source availability blocked while generated source-availability records report available. | Conflicting | [`ResourceRef` README](https://github.com/sudoprivacy/sudostack/blob/aec52e9dc5438946dd83d5c56c07149a4800120a/contracts/common/v1/resource-ref/README.md#L22-L30); [source availability](https://github.com/sudoprivacy/sudostack/blob/aec52e9dc5438946dd83d5c56c07149a4800120a/manifests/operations/source-availability.json#L6-L35) |
| C-03 | The candidate manifest declares `draft-frozen` while its actual producer/consumer arrays are empty; ADR005-FREEZE-01 says absence of a real consumer is a fail-closed `unfrozen` condition. | Conflicting | [freeze clause](https://github.com/sudoprivacy/sudostack/blob/aec52e9dc5438946dd83d5c56c07149a4800120a/docs/adr/ADR-005-product-contract-versioning.md#L162-L178); [candidate package/support state](https://github.com/sudoprivacy/sudostack/blob/aec52e9dc5438946dd83d5c56c07149a4800120a/manifests/releases/0.2.0-candidate.gen.json#L275-L287) |

Do not resolve C-01 through C-04 by choosing the more convenient text or test. Assembly must use the exact generated/verified candidate bytes and separately close the freeze and production-boundary conflicts. C-01/C-02 may have a documented, authorized stale-document disposition tied to exact evidence; C-03/C-04 cannot be waived locally because they concern the ADR freeze rule and an absent production call. None of these conflicts changes the candidate's unpublished/not-deployed state.
...
|---|---|---|
| SudoStack/F1 | Main `44b4e9de…` declares `@sudo/contracts@0.1.0`; PR 18 declares private `@sudo/contracts@0.2.0`, Node `>=22.21.0`, generated source/release manifests, and package-level smoke, but no executable selected-profile deployment or live artifact digest. | Candidate source identity is Confirmed-Code; package/image/binary digest and deployment are Unknown. |
...
9. C-03 and C-04 are closed with authoritative code/manifest evidence. C-01/C-02 are corrected or have an explicitly authorized stale-document disposition tied to exact evidence; an inventory-local waiver cannot override the ADR freeze rule or substitute tests for a production call;
