...
Source cut: 2026-09-20; the standalone support evidence is anchored to SudoStack baseline `93e72878fc7b246d7f7f2418ff19ac41a02b5a66`, support-operation SHA-256 `21508ae15abf7dda253cc0d0de9f71afe500f2da98577809297302e7987c9dd5`, and unchanged candidate-manifest SHA-256 `2a13fdaf59d26bc4af7db4a2f58f6956555b1d61b1da81bd03b4b879fb1c548c`, recorded in [`consumer-support.json`](../../manifests/operations/consumer-support.json).

| Item | Current state | Evidence status |
|---|---|---|
| ADR-005 | `Proposed` | Confirmed-Code |
| Contract baseline | Manifest-declared `draft-frozen`; standalone evidence proves the Moss boundary, but the activated package still carries empty support arrays | Conflicting |
...
The G3 **included consumer target is Moss/`ZoneId` only**. The standalone verifier binds the exact six-file feature content, merge parents, installed package pin, production caller, positive/negative/no-side-effect tests, and hosted checks. Operator configuration is the authoritative input writer; Moss validates and binds the value and produces the embedded launch argument; Nexus persisted topology is the canonical runtime store. External topology is excluded. The activated package remains byte-identical to baseline and still embeds empty producer/consumer arrays, so C-03 remains open rather than treating out-of-package evidence as an in-place `0.2.0` metadata update. Nexus owner artifacts remain part of the source closure, but `ResourceRef` has no production provider/consumer and is excluded from mixed-version and deployment claims. SudoWork, sudocode, and `sudowork-server` are not candidate consumers in this scope. This code-level evidence does not satisfy any live B0, release, deployment, or rollback gate.
...
| E-02 | SudoStack baseline `93e72878fc7b246d7f7f2418ff19ac41a02b5a66` contains the exact activated/available candidate; the current transition adds only standalone support evidence, its verifier/CI wiring, and documentation. | Confirmed-Code | [consumer-support operation](../../manifests/operations/consumer-support.json); [unchanged candidate manifest](../../manifests/releases/0.2.0-candidate.gen.json) |
| E-03 | The candidate records exact owner revisions and retains `candidate_unpublished`, `not_deployed`, and empty package producer/consumer arrays byte-for-byte; Moss integration evidence is recorded only in the standalone operation. | Confirmed-Code | [unchanged candidate manifest](../../manifests/releases/0.2.0-candidate.gen.json); [standalone support evidence](../../manifests/operations/consumer-support.json) |
...
| E-13 | The support verifier proves the complete npm pack file set and every candidate/runtime/Schema/fixture/source-lock/source-closure/compatibility byte equal SudoStack baseline `93e7287…`; only the standalone operation, its verifier/tests, workflow, and documentation change. | Confirmed-Code | [support verifier](../../tools/contracts/support.mjs); [support mutation suite](../../tools/contracts/support.test.mjs) |
| C-03 | Standalone evidence now proves a real Moss `ZoneId` consumer, but the activated `@sudo/contracts@0.2.0` package and candidate manifest intentionally remain byte-identical to baseline with empty embedded producer/consumer arrays. Closing this metadata gap requires a separately versioned candidate and consumer repin rather than rewriting `0.2.0`. | Conflicting; assembly-blocking | [freeze clause](../adr/ADR-005-product-contract-versioning.md#7-draft-frozen-baseline); [standalone support operation](../../manifests/operations/consumer-support.json); [unchanged candidate manifest](../../manifests/releases/0.2.0-candidate.gen.json) |
...
C-04 is resolved by exact production-caller evidence. C-03 remains assembly-blocking because the immutable `0.2.0` package/candidate cannot gain embedded support metadata without a new candidate/version and Moss repin. Assembly and G3 also remain blocked by the 13 Unknown live inputs, built-artifact identity, mixed-version smoke, and exact rollback rehearsal; the support operation does not change `candidate_unpublished`, `not_deployed`, `ResourceRef`/`ZonePath` deferral, or the zero `Confirmed-Deployment` count.
...
|---|---|---|
| SudoStack/F1 | Main baseline `93e7287…` declares private `@sudo/contracts@0.2.0`, Node `>=22.21.0`, generated source/release manifests, and package-level smoke; the current support operation adds verifier-authenticated Moss `ZoneId` metadata, but no executable selected-profile deployment or live artifact digest. | Candidate source identity and code-level support are Confirmed-Code; package/image/binary digest and deployment are Unknown. |
...
9. the C-04 resolution operation, exact Moss revisions, paths, hashes, package pin, boundary assertions, and hosted-check evidence still pass strict verification, and C-03 is closed only by a separately versioned candidate/support-metadata update plus consumer repin; an inventory-local waiver cannot replace either proof;
