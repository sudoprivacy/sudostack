...
> **Manual gate:** the operational requirements in this document are currently `[enforced_by: none]`. Repository CI verifies this source-only template's invariants, but it does not validate completed live records or grant assembly/G3 approval; a syntactically valid or fully populated JSON file is not by itself approval.
...
Source cut: 2026-09-20. The historical standalone support evidence is anchored to SudoStack baseline `93e72878fc7b246d7f7f2418ff19ac41a02b5a66`, support-operation SHA-256 `21508ae15abf7dda253cc0d0de9f71afe500f2da98577809297302e7987c9dd5`, and unchanged `0.2.0` candidate-manifest SHA-256 `2a13fdaf59d26bc4af7db4a2f58f6956555b1d61b1da81bd03b4b879fb1c548c`, recorded in [`consumer-support.json`](../../manifests/operations/consumer-support.json). The separately versioned `0.2.1` content candidate is staged and unfrozen; it has no content-commit SHA or new Moss evidence yet.

| Item | Current state | Evidence status |
|---|---|---|
| ADR-005 | `Proposed` | Confirmed-Code |
| Contract baseline | Historical `0.2.0` manifest declares `draft-frozen`, while its package support arrays remain empty; staged `0.2.1` is explicitly `unfrozen` pending Moss repin and later activation evidence | Conflicting |
| F1 Contract artifact | `@sudo/contracts@0.2.1` content staged; `candidate_unpublished`; candidate revision pending | Confirmed-Code |
...
| Confirmed Contract boundary | Moss, `ZoneId` only, at `NexusManager.start()` for embedded Nexus; exact package pin, immutable binding, one `--cluster-init` argument, real-caller tests, and default CI are independently verifier-bound | Confirmed-Code; standalone evidence, not package metadata or deployment evidence |
...
| `ZonePath` | Owner source dependency for the deferred `ResourceRef` family; no runtime adoption claim | Confirmed-Code; deferred |
...
The source references below span merged owner, distribution, and Moss consumer pull requests plus strict post-activation support metadata. VCS status, code existence, tests, package installation, or a green pull request does not establish that a customer environment runs those bytes. Only evidence gathered from the selected live environment may use `Confirmed-Deployment`.
...
| Historical exact-pin/derive/distribute candidate | private package `@sudo/contracts@0.2.0`; content `30da0ddd953268ff8a9a0f0980276300ab153003`, activation `60bd8dda6fb2d348ec8571b9b1a4eaa535e36dc5`, source availability `aec52e9dc5438946dd83d5c56c07149a4800120a`, support-evidence baseline `93e72878fc7b246d7f7f2418ff19ac41a02b5a66` | Confirmed-Code; standalone support evidence is outside the package, whose candidate/compatibility/support arrays remain byte-identical to baseline; `candidate_unpublished`; `not_deployed` | [consumer-support operation](../../manifests/operations/consumer-support.json), [unchanged candidate manifest](../../manifests/releases/0.2.0-candidate.gen.json) |
| Staged successor candidate | private package `@sudo/contracts@0.2.1`; runtime/Schema/fixture/validator/owner-source bytes equal the immutable `0.2.0` baseline; content revision is pending until C exists | Confirmed-Code; `unfrozen`; empty candidate support; `candidate_unpublished`; `not_deployed`; no Moss `0.2.1` pin yet | [content-stage record](../design/contracts-0.2.1-content-stage.md), [staged candidate manifest](../../manifests/releases/0.2.1-candidate.gen.json) |
| Included consumer for `ZoneId` | feature `sudoprivacy/moss@71be276ebe1ea58882db9caaa6c42d8df5a67ad0`, merged to `dev` as `604a78ee27b290fc40e85fe0eb469e5a4cb13c91` | Confirmed-Code; PR 276 merged; `NexusManager.start()` validates the immutable binding and emits exactly one `--cluster-init <id>` for embedded mode; external mode excluded; default CI passed; no release/deployment claim | [Moss PR 276](https://github.com/sudoprivacy/moss/pull/276), [exact support record](../../manifests/operations/consumer-support.json) |

The G3 **included consumer target is Moss/`ZoneId` only**. Historical standalone verification binds the exact six-file Moss feature content, merge parents, installed `0.2.0` package pin, production caller, positive/negative/no-side-effect tests, and hosted checks. Operator configuration is the authoritative input writer; Moss validates and binds the value and produces the embedded launch argument; Nexus persisted topology is the canonical runtime store. External topology is excluded. The historical activated package remains byte-identical to its baseline and still embeds empty producer/consumer arrays. The `0.2.1` content stage likewise has empty arrays and treats the old Moss evidence as historical-only. C-03 therefore remains open until Moss exact-pins content commit C at M and a later package-external activation A binds and verifies C+M without modifying C's package bytes. Nexus owner artifacts remain part of the source closure, but `ResourceRef` has no production provider/consumer and is excluded from mixed-version and deployment claims. SudoWork, sudocode, and `sudowork-server` are not candidate consumers in this scope. This code-level evidence does not satisfy any live B0, release, deployment, or rollback gate.
...
| E-02 | SudoStack baseline `93e72878fc7b246d7f7f2418ff19ac41a02b5a66` contains the exact activated/available `0.2.0` candidate; its later support transition added only standalone evidence, verifier/CI wiring, and documentation. The separately versioned `0.2.1` content stage adds no adoption evidence. | Confirmed-Code | [historical consumer-support operation](../../manifests/operations/consumer-support.json); [content-stage record](../design/contracts-0.2.1-content-stage.md) |
| E-03 | The historical `0.2.0` package retains `candidate_unpublished`, `not_deployed`, and empty package producer/consumer arrays byte-for-byte. The staged `0.2.1` candidate is `unfrozen`, also has empty arrays, and classifies prior Moss integration as historical-only pending M and A. | Confirmed-Code | [historical candidate manifest](../../manifests/releases/0.2.0-candidate.gen.json); [staged candidate manifest](../../manifests/releases/0.2.1-candidate.gen.json) |
...
| E-06 | Moss feature `71be276…`, integrated as `604a78e…`, makes the installed ZoneId validator part of the real embedded `NexusManager.start()` path, preserves an immutable binding, emits one `--cluster-init <id>`, and rejects external-mode ownership. | Confirmed-Code | [Moss PR 276](https://github.com/sudoprivacy/moss/pull/276); [exact paths, hashes, pin, boundary, and checks](../../manifests/operations/consumer-support.json) |
...
| E-09 | SudoStack requires Node `>=22.21.0`; Moss PR 276 retained the exact package pin and passed lint, server tests, and amd64 build, but the actual running runtime remains Unknown. | Confirmed-Code | [`package.json`](../../package.json); [Moss PR 276 checks](https://github.com/sudoprivacy/moss/pull/276/checks) |
...
| E-12 | SudoStack includes temporary-directory package pack/install/import/typecheck smoke, and Moss’s exact consumer revision has real-caller tests plus packaged-server/runtime-image checks. These are executable code-level checks, not evidence that the selected customer environment passed. | Confirmed-Code | [package smoke](../../tools/contracts/package-smoke.mjs); [support evidence](../../manifests/operations/consumer-support.json) |
| E-13 | The immutable `0.2.0` baseline records all 42 packed files plus protected source/provenance and historical records; current gates replay the original activation/availability/support chain in a detached historical checkout and require the staged `0.2.1` package to differ only by version, compatibility metadata, and its new candidate manifest. | Confirmed-Code | [immutable package baseline](../../compatibility/baselines/0.2.0-package.json); [history verifier](../../tools/contracts/history-0.2.0.mjs); [package smoke](../../tools/contracts/package-smoke.mjs) |
| C-03 | Historical standalone evidence proves a real Moss `ZoneId` consumer for `0.2.0`. The staged `0.2.1` candidate deliberately remains `unfrozen` with empty support arrays and no Moss `0.2.1` pin. C prepares installable content only; M must exact-pin C and prove the same real boundary; A must then bind C+M outside the package before deciding the metadata gap. | Conflicting; assembly-blocking | [freeze clause](../adr/ADR-005-product-contract-versioning.md#7-draft-frozen-baseline); [content-stage record](../design/contracts-0.2.1-content-stage.md); [historical support operation](../../manifests/operations/consumer-support.json) |
...
| C-02 | The ResourceRef README said remote owner-source verification was blocked after the source-availability operation recorded `available`. | Resolved by docs-only F1 revision `b966826…`; the README now says exact owner bytes are remotely available and explicitly separates availability from registry release, adoption and deployment. | [corrected ResourceRef status](https://github.com/sudoprivacy/sudostack/blob/b966826edada1c2c1086ab6ee46cec9f85a398b5/contracts/common/v1/resource-ref/README.md#L34-L42); [availability record](../../manifests/operations/source-availability.json) |
| C-04 | Moss PR 272 tested `buildNexusArgs` directly while the real embedded startup caller omitted `clusterInit` and external mode bypassed the boundary. | Resolved by Moss PR 276: feature `71be276…`, merge `604a78e…`, six exact reviewed files, an immutable binding at `NexusManager.start()`, exactly one embedded `--cluster-init <id>`, explicit external exclusion, and successful lint/server-tests/build checks; publication was skipped by design. | [Moss PR 276](https://github.com/sudoprivacy/moss/pull/276); [exact support evidence](../../manifests/operations/consumer-support.json) |

C-04 is resolved by exact production-caller evidence. C-03 remains assembly-blocking: C only stages byte-identical `0.2.1` content, M must exact-pin C and produce candidate-specific boundary evidence, and package-external A must bind C+M before deciding closure. Assembly and G3 also remain blocked by the 13 Unknown live inputs, built-artifact identity, mixed-version smoke, and exact rollback rehearsal; neither C nor the historical support operation changes `candidate_unpublished`, `not_deployed`, `ResourceRef`/`ZonePath` deferral, or the zero `Confirmed-Deployment` count.
...
|---|---|---|
| SudoStack/F1 | Historical baseline `5a2a531…` contains private `@sudo/contracts@0.2.0` and the standalone Moss support evidence. The current source stages private `0.2.1`, requiring identical runtime/Schema/fixture/source bytes and leaving support pending; neither tree contains an executable selected-profile deployment or live artifact digest. | Candidate source identities and historical code-level support are Confirmed-Code; C has no new consumer evidence, and package/image/binary digest plus deployment remain Unknown. |
| Moss | Base inspection anchor `be38bec235c4dea202e32791f4efd20529e18d20` declares package `2.1.88`, independent server/runtime release identities, PostgreSQL migrations `1`–`4`, and installer rollback of config/current symlink—not a database rollback. PR 276 is the only included consumer and binds the exact candidate at the embedded production startup boundary. | Record the running Moss revision, installed Contract integrity/digest, server and separate session runtimes, image digest, applied migration state, config reference, actual boundary smoke, and DB restore evidence. [Package](https://github.com/sudoprivacy/moss/blob/604a78ee27b290fc40e85fe0eb469e5a4cb13c91/package.json), [exact support evidence](../../manifests/operations/consumer-support.json) |
...
7. the target consumer set remains exactly Moss/`ZoneId`; the verifier-backed production startup path passes the selected ZoneId through the installed validator and immutable binding to exactly one embedded `--cluster-init` argument, while external mode remains excluded. SudoWork, sudocode, and Nexus runtime `ResourceRef` adoption require separate accepted work items;
8. `ResourceRef` and its runtime `ZonePath` use remain deferred and excluded from the demo claim;
9. the C-04 historical resolution operation and its exact Moss revisions, paths, hashes, `0.2.0` pin, boundary assertions, and hosted-check evidence still pass in the immutable historical checkout; C-03 remains open through content stage C and repin/evidence stage M, and may close only when package-external activation A binds and verifies exact C+M without changing C's package bytes or requiring another Moss repin;
