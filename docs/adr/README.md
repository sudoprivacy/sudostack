...
**行为层刻意不写「conformance suite」**：为行为类条款写一套场景向量，等于把状态机实现第二遍 ——
它会同时偏离两边，而且因为是绿的所以没人怀疑。向量只适合跨语言的纯函数规则（`zone-id` 就是理想情形）。
判据与手段见根目录 [README](../../README.md#契约怎么落地先读这条)。

### 三条贯穿所有 ADR 的总则

**1. 每条规范性条款带一个 `enforced_by`，指向一个真实存在的产物；没有就写 `enforced_by: none`。**

没有第三种状态。一条读起来像规范、实际没人校验的句子，比不写更糟 —— 它让读者以为这件事有人管着。

这条本身也得能失败，否则它只是另一句散文：`enforced_by` 指向的测试 id / 生成常量 / 类型必须真的能被找到，
...
**根因（nexus-2 的表述，比我原来的准）**：当一个 API 只能**带内**作答时，「没有」和「我判断不了」会塌缩成同一个值，
而下游每一个调用方都继承这份歧义。`exists()` 返回 `false`、`readdir` 返回 `[]`、流读返回
`eof=true, next_offset=0` —— 三者都是合法答案，也都是失败时会给出的答案。所以判据不是「有没有报错」，
而是**这个返回类型有没有能力把两者分开**；没有能力，就得改返回类型，而不是加注释。

**3. 在 id 被「选定」处拒绝，在 id 被「习得」处告警。**

这条是做 zone-id 时长出来的，一般性比它的出处大。同一个校验放错位置会从保护变成伤害：

- **选定**（operator 敲 `--cluster-init`、CLI 传 `--zone-id`、服务运行时申请新 zone）—— **拒绝**。
  此时什么都还没建，拒绝零代价，而 id 一旦落盘就成了该 zone 内所有路径的第一段，改不了。
- **习得**（从磁盘恢复、从别的节点物化一个已存在的副本）—— **告警，放行**。

反例值得记住：`ZoneRegistry::create_zone` 看起来是最好的卡点，因为一切都汇流于此 —— 而这恰恰是它错的原因。
它同时承担「物化一个别处已存在的 zone 的本地副本」。在那里拒绝，会让集群里某一个不合规的 id
**阻止本节点复制一个所有人都有的 zone** —— 一个披着校验外衣的分区，而且是静默的。

同理，时间维度上：首次启动是「选定」，重启是从磁盘「习得」。所以首启拒绝、重启告警，
不是两条规则，是同一条规则作用在两个轴上。
